...
function tariffFeatures(tariff: Tariff): string[] {
  const features = tariff.features as unknown;
...
export function BuxSoftApp({ data }: { data: CmsData }) {
  ...
  const selectedTariff =
    categoryTariffs.find((tariff) => tariff.id === tariffId) || categoryTariffs[0];
  ...
  function chooseCategory(category: Category) {
    ...
    const nextTariff = sortItems(
      data.tariffs.filter((tariff) => tariff.category_id === category.id)
    )[0];
  ...
  return (
    <main className="bg-premium min-h-screen overflow-hidden">
        ...
        <nav className="glass mb-12 flex items-center justify-between rounded-full px-4 py-3 sm:px-5">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-2xl bg-white text-sm font-black text-slate-950">
              {setting(data, "logo_letter", "B")}
            </div>

            <span className="font-semibold tracking-tight">
              {setting(data, "site_title", "BuxSoft")}
            </span>
          </div>

          <a
            href="#calculator"
            className="rounded-full bg-white px-4 py-2 text-xs font-bold text-slate-950 transition hover:bg-cyan-100 sm:text-sm"
          >
          ...
          <div>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-cyan-100">
              ...
              {setting(
                data,
                "hero_badge",
                "Soliq xavflarini kamaytiring, hisobni mutaxassislarga topshiring"
              )}
            </div>
            ...
            <h1 className="max-w-5xl overflow-visible pb-3 text-balance text-[44px] font-bold leading-[1.12] tracking-[-0.035em] text-white drop-shadow-[0_24px_70px_rgba(255,255,255,0.08)] sm:text-6xl lg:text-[76px]">
              {setting(data, "hero_title", "Soliq tekshiruvlariga oldindan tayyorlaymiz")}
            </h1>
            ...
            <p className="mt-7 max-w-2xl text-base leading-8 text-slate-300 sm:text-xl">
              {setting(
                data,
                "hero_subtitle",
                "Buxgalteriya, soliq hisobotlari va hujjatlaringizni tartibga keltiramiz. Faoliyatingizga mos tarifni tanlang va xizmat narxini real vaqtda hisoblang."
              )}
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a
                href="#tariffs"
                className="rounded-full border border-white/15 px-5 py-3 text-sm font-bold text-white/90 transition hover:bg-white/10"
              >
      ...
      <section
        id="calculator"
        className="relative mx-auto max-w-7xl scroll-mt-8 px-4 py-12 sm:px-6 lg:px-8"
      >
        <p className="text-sm font-bold uppercase tracking-[.28em] text-cyan-300">
          CMS calculator
        </p>
      ...
      >
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            <p className="text-sm font-bold uppercase tracking-[.28em] text-violet-300">
              Dynamic pricing
            </p>
            <h2 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">
              Dinamik tariflar
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-3">
          {categoryTariffs.length ? (
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedTariff?.id === tariff.id &&
                    "border-violet-300/70 ring-2 ring-violet-300/70"
                )}
              >
                {tariff.badge && (
                ...
                <div
                  className={cn(
                    "mb-6 h-2 w-24 rounded-full bg-gradient-to-r",
                    tariff.gradient || "from-cyan-400 to-violet-500"
                  )}
                />
                ...
                <div className="mt-5 text-3xl font-black">
                  {money(tariff.price, currency)}
                </div>

                {tariff.old_price ? (
                  <div className="mt-1 text-sm text-slate-500 line-through">
                    {money(tariff.old_price, currency)}
                  </div>
                ) : null}
            ...
            <div className="glass rounded-[2rem] p-6 text-slate-300 lg:col-span-3">
              Bu faoliyat turi uchun tariflar hali Google Sheetsga kiritilmagan.
            </div>
          )}
        </div>
      </section>
      ...
      <section
        id="services"
        className="relative mx-auto grid max-w-7xl scroll-mt-8 gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_420px] lg:px-8"
      >
        <div className="glass rounded-[2rem] p-6">
          <h2 className="text-3xl font-black">Qo‘shimcha xizmatlar</h2>
          <p className="mt-2 text-sm text-slate-400">
            Xizmat tanlang — narx avtomatik yangilanadi.
          </p>
          ...
          <div className="mt-6 grid gap-3">
            {availableServices.length ? (
              availableServices.map((service) => (
                <label
                  key={service.id}
                  className="flex cursor-pointer items-start gap-4 rounded-2xl border border-white/10 bg-white/5 p-4 transition hover:bg-white/10"
                >
                  <input
                    type="checkbox"
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-slate-300">
                Bu tarif uchun qo‘shimcha xizmatlar hali kiritilmagan.
              </div>
            )}
          </div>
        </div>
        ...
        <form onSubmit={onSubmit} className="glass rounded-[2rem] p-6">
          ...
          >
            ...
            {status === "loading"
              ? "Yuborilmoqda..."
              : setting(data, "cta_text", "Yuborish")}
          </button>

          {status === "success" && (
            <div className="mt-4 rounded-2xl bg-emerald-400/10 p-4 text-sm text-emerald-200">
              {setting(data, "lead_success_title", "Qabul qilindi")}.{" "}
              {setting(data, "lead_success_text", "Tez orada bog‘lanamiz.")}
            </div>
          )}

          {status === "error" && (
            <div className="mt-4 rounded-2xl bg-red-400/10 p-4 text-sm text-red-200">
              Xatolik yuz berdi. Telefon yoki Telegramni tekshirib qayta urinib
              ko‘ring.
            </div>
          )}
        </form>
            ...
            {data.testimonials.map((item) => (
              <div key={item.id} className="glass rounded-[2rem] p-6">
                <div className="flex gap-1 text-amber-300">
                  {Array.from({
                    length: Math.max(1, Math.min(5, Number(item.rating || 5))),
                  }).map((_, index) => (
          ...
          <div className="flex flex-col justify-between gap-5 md:flex-row">
            <div>
              ...
              <p className="mt-2 max-w-xl">
                {setting(data, "footer_text", "Buxgalteriya xizmatlari")}
              </p>
            </div>
